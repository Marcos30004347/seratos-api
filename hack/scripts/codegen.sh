...
docker build -f "${CURRENT_DIR}/hack/docker/codegen.dockerfile" -t "${IMAGE_NAME}" "${REPO_DIR}"
...
cmd1="./generate-internal-groups.sh "deepcopy,defaulter,conversion,informer,lister,client" \
    "$PROJECT_MODULE/pkg/generated" \
    "$PROJECT_MODULE/pkg/apis" \
    "$PROJECT_MODULE/pkg/apis" \
    $CUSTOM_RESOURCE_NAME:$CUSTOM_RESOURCE_VERSION"
