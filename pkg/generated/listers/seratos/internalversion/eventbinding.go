...
type EventBindingLister interface {
	...
	List(selector labels.Selector) (ret []*seratos.EventBinding, err error)
...
// List lists all EventBindings in the indexer.
func (s *eventBindingLister) List(selector labels.Selector) (ret []*seratos.EventBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*seratos.EventBinding))
	})
	return ret, err
}
...
type EventBindingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*seratos.EventBinding, error)
...
// List lists all EventBindings in the indexer for a given namespace.
func (s eventBindingNamespaceLister) List(selector labels.Selector) (ret []*seratos.EventBinding, err error) {
...
// Get retrieves the EventBinding from the indexer for a given namespace and name.
func (s eventBindingNamespaceLister) Get(name string) (*seratos.EventBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(seratos.Resource("eventbinding"), name)
	}
	return obj.(*seratos.EventBinding), nil
}
