...
type SidecarLister interface {
	...
	List(selector labels.Selector) (ret []*seratos.Sidecar, err error)
...
// List lists all Sidecars in the indexer.
func (s *sidecarLister) List(selector labels.Selector) (ret []*seratos.Sidecar, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*seratos.Sidecar))
	})
	return ret, err
}
...
type SidecarNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*seratos.Sidecar, error)
...
// List lists all Sidecars in the indexer for a given namespace.
func (s sidecarNamespaceLister) List(selector labels.Selector) (ret []*seratos.Sidecar, err error) {
...
// Get retrieves the Sidecar from the indexer for a given namespace and name.
func (s sidecarNamespaceLister) Get(name string) (*seratos.Sidecar, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(seratos.Resource("sidecar"), name)
	}
	return obj.(*seratos.Sidecar), nil
}
